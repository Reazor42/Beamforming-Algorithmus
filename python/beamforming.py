...
def removeWhitePixels(gifpath):
    ...
    #hier gucken
    images[0].save(path, save_all=True, append_images=images[1:], optimize=True, duration=1000, loop=0, disposal=2,
                   transparency=0)
...
for a in avgt.result(1):
    ...
    Lm = acoular.L_p(pm)
    plt.axis("off")
    ax.imshow(Lm.T, vmin=Lm.max() - 25, vmax=Lm.max(), origin='lower', cmap='plasma', extent=rg.extend(),
...
animation = cam.animate(blit=True, repeat=False, interval=1000)
...
os.system(
    'ffmpeg -hide_banner -loglevel panic -i {0} -i {1} -filter_complex "[1]format=argb,colorchannelmixer=aa=0.7[front];[front]scale=2230:1216[next];[0][next]overlay=x=-155:y=0,format=yuv420p" {2}'.format(
        "data/data_1/Video_1_4m_40db.mp4", "img/gifoverlay.gif", "img/overlay.mp4"))
print("Done.")
